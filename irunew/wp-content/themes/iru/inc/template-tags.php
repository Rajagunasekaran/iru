...
if (!function_exists('bootstrapBasicPagination')) {
	 ...
	 * @return string the content already echo
	 */
	function bootstrapBasicPagination($pagination_align_class = 'pagination-center pagination-row iru-pagination') 
	{
		global $wp_query;
			$big = 999999999;
       ...
       $current=get_query_var('paged');
			...
			if (is_array($pagination_array) && !empty($pagination_array)) {
				...
				echo '<div class="page_of">Page '.$current." of ".$wp_query->max_num_pages.'</div>';
				echo '</nav>';
			}
...
if (!function_exists('bootstrapBasicPostOn')) {
	...
	function bootstrapBasicPostOn() 
	{
		...
		printf(__('<span class="posted-on"> %1$s</span><span class="byline"> by %2$s</span>', 'bootstrap-basic'),
